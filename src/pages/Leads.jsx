    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8 space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-[13px] text-stone-400">{readOnly ? `${leads.length} prospects · scored by Aurelius` : hasSearched ? `${leads.length} prospects · scraped and scored by Aurelius` : 'Run a search to load leads'}</p>
        </div>
      ...
      ) : !readOnly && !hasSearched && !showHistory ? (
        <div className="text-center py-20 text-stone-400 text-sm">No leads loaded. Use the search above to find leads.</div>
      ) : leads.length === 0 ? (
        <div className="text-center py-20 text-stone-400 text-sm">
          No leads yet{readOnly ? '.' : '. Use the search above to have Aurelius find prospects for you.'}
        </div>
      ) : view === 'pipeline' ? (
        <LeadPipeline leads={leads} onMove={moveLead} onOpen={setOpenLead} readOnly={readOnly} />
      ) : (
        <LeadTable leads={leads} onOpen={setOpenLead} onBulkStatus={bulkStatus} selected={selected} setSelected={setSelected} readOnly={readOnly} />
      )}

      <LeadDetailDrawer lead={openLead} onClose={() => setOpenLead(null)} readOnly={readOnly} onUpdated={(l) => { setLeads((ls) => ls.map((x) => (x.id === l.id ? l : x))); }} />
    </div>
  );
}